...
class PecaEnviada {
  ...
  async listagem(page, browser, arr = [], i = 0) {
    try {
      ...
      if (detalhamentoValue.includes("Peça encaminhada,  previsão de entrega e rastreio")) {
        ...
        // Selecione a opção "Despachada para o técnico"
        const option = await page.$("ul.dropdown-list li:nth-child(21)");
        await option.click();
        ...
        await page.keyboard.type(
          `Peça encaminhada,  previsão de entrega e rastreio disponíveis em 2 dias úteis`
        );
        await page.waitForTimeout(2000);
        await rep.updateGatilhoSinistros(arr[i].id);
        //Salvar
