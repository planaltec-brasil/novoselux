...
class AguardandoRetornoTec {
  ...
  async listagem(page, browser, arr = [], i = 0) {
    try {
      ...
      if (detalhamentoValue.includes("Documentação recebida, previsão de agendamento com o prestador")) {
        ...
        // Selecione a opção "Aguardando Aceite do Técnico"
        ...
        await page.keyboard.type(
          `Documentação recebida, previsão de agendamento com o prestador ${AGFormatado}.`
        );
        await page.waitForTimeout(2000);
        await rep.updateGatilhoSinistros(arr[i].id);
        //Salvar
...
const rep = new AguardandoRetornoTec();
rep.logando();
