...
class Vinculacao {
  ...
  async listagem(page, browser, arr = [], i = 0) {
    try {
      ...
      try {
        var obscorrigida = obs.split("|")[1].trim();
        obscorrigida = obscorrigida.replaceAll("<b>", "");
        obscorrigida = obscorrigida.replaceAll("</b>", "");
        console.log(obscorrigida);
      } catch (error) {
        await teste.listagem(page, browser, arr, ++i);
        return;
      }
      ...
      const statusValue = await page.$eval(
        "#main-content > ng-component > div > div:nth-child(4) > div:nth-child(3) > div:nth-child(1) > div:nth-child(10) > span",
        (element) => element.textContent.trim()
      );

      console.log(statusValue); // Exibe o conteúdo no console
      if(statusValue == "Pedido faturado"){
        await page.waitForTimeout(5000);
        teste.listagem(page, browser, arr, ++i);
        return;
      }
